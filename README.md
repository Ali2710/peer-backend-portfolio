# Backend Case Studies — Peer Network (Open Source PRs)

This repository is a narrative portfolio built from my merged pull requests in **Peer Network**.
Each case study follows a consistent format: **Problem → Solution → Tech → Outcome** and links to the original PRs for verification.

## Case Studies
1. Personalized Feed Ranking & Social Graph (B)
2. Moderation & Reporting Revamp (C)
3. Tokenomics & Fee Model Migration (D)
4. Media Pipeline: Auto Covers + Illegal Content Hardening (E)
...
- I can ship product features (feed ranking, action pricing, onboarding).
- I can evolve data models safely (reporting unification, fee model migrations).
- I can harden platform behavior (media covers + illegal content handling).
- I can reduce technical debt at scale (PHPStan + Rector + strict types + constants consolidation).
